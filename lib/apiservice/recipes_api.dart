...
class RecipesApi {
  RecipesApi(BaseApiService apiService);

  // Kategoriler
  Future<List<Kategori>> fetchKategoriler() async {
    final response = await http.get(Uri.parse('${BaseApiService.baseUrl}/api/Kategori'));
  ...
  Future<List<TarifOnizleme>> fetchTarifOnizleme() async {
    final response = await http.get(Uri.parse('${BaseApiService.baseUrl}/api/tarifonizleme'));
    if (response.statusCode != 200) {
  ...
  Future<List<Malzeme>> fetchMalzemeler() async {
    final response = await http.get(Uri.parse('${BaseApiService.baseUrl}/api/malzeme'));
    if (response.statusCode != 200) {
  ...
  Future<TarifDetay> getTarifDetay(int id) async {
    final uri = Uri.parse('${BaseApiService.baseUrl}/api/tarif/$id');
  ...
  Future<List<TarifOneriSonuc>> tarifOneriGetir(List<int> malzemeIdler) async {
    final uri = Uri.parse('${BaseApiService.baseUrl}/api/malzemebytarif');
